...
model = load_model('yes_no_final.h5')
...
#read image
img = 'I:/BBBB/final_output_data/808.jpg'
...
#tumor identification in MRI

class_names = ['yes', 'no']
width = 256
height = 256

type_2 = preprocessing.image.load_img(img, target_size=(width, height))

type_2_X = np.expand_dims(type_2, axis=0)

predictions = model.predict(type_2_X)

print('Does this patient has a tumor ? : {}'.format(class_names[np.argmax(predictions)]))
...
# Calculate area of the tumor in left and right segments
